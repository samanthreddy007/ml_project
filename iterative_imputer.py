# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 19:11:43 2019
...
cat_data_new.dropna(thresh=len(cat_data_new.index)/2, axis=1, inplace=True)
X_incomplete=np.matrix(cat_data_new)
#X=[[np.nan,np.nan,np.nan],[np.nan,np.nan,np.nan],[np.nan,4,np.nan]]
#X_incomplete=np.matrix(X)
X_imputed_iterative = IterativeImputer().fit_transform(X_incomplete)
X_imputed_simple_fill=SimpleFill().fit_transform(X_incomplete)

#X_imputed_nnm_fill = NuclearNormMinimization().fit_transform(X_incomplete)